...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  startAnalysis: async () => {
    ...
    // 2. LIVE PRODUCTION ROUTE WITH RETRIES AND TIMEOUT FAULTS
    try {
      // Progress pipeline visually alongside network execution
      const stepInterval = setInterval(() => {
        const curr = get().analysisStep;
        if (curr < 5) {
          set({ analysisStep: (curr + 1) as any });
        }
      }, 1000);
      ...
      while (retryCount <= maxRetries) {
        try {
          response = await axios.post(`${API_BASE_URL}/analyze`, {
            ...
            location: currentLocation
          }, { timeout: 6000 }); // 6s timeout trigger
          break;
        } catch (err: any) {
          retryCount++;
          if (retryCount > maxRetries) {
            clearInterval(stepInterval);
            throw err; // throw and let catch handles the error card layouts
          }
          // Exponential backoff retry wait: 1s, 2s
      ...
      // Distinguish between pure lost connection and timeout bounds
      const isTimeout = e.code === 'ECONNABORTED' || e.message?.includes('timeout');
      const isNetworkErr = !e.response && !isTimeout;

      if (isNetworkErr) {
        set({ 
          error: 'NO_INTERNET',
          demoMode: true, // switch to demo mode automatically
          isAnalyzing: false,
          analysisStep: 0
        });
      } else {
